...
ShipMvRSystem::ShipMvRSystem(ECS::Registry & registry) :
    _registry(registry)
{}

std::vector<Info> ShipMvRSystem::callSystem(std::vector<std::any> & params) {
    std::vector<Info> map;
    ...
    for (int i = 0; i < positions.size() && i < moving.size() && i < rectangles.size(); i++) {
        ...
        if (pos && move && rect) {
            ...
            if (vpos.getX() + vpos.getW() + vmove.getSpeed() <= WINDOW_WIDTH) {
                vpos.setX(vpos.getX() + vmove.getSpeed());
                vrect.setX(SHIP_RIGHT_X);
