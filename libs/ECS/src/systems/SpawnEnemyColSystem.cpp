...
SpawnEnemyColSystem::SpawnEnemyColSystem(Registry & registry) :
    _registry(registry)
{}

std::vector<ECS::Info> SpawnEnemyColSystem::callSystem(std::vector<std::any> & params) {
    std::vector<ECS::Info> list;
        srand (time(NULL));
        int posX = rand() % (WINDOW_WIDTH / 2) - ENEMY_WIDTH + (WINDOW_WIDTH / 2);
        int posY = 15;

        for (int nbEnemies = 0; nbEnemies != 3; nbEnemies++, posY += ENEMY_HEIGHT + 15) {
